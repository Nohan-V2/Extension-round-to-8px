...
function roundRemValue(remValue: number): number {
  // Arrondir à 2 décimales pour éviter les erreurs d'arrondi
...
function processDocument(document: vscode.TextDocument): vscode.TextEdit[] {
  ...
  const propertyRegex =
    /(margin|margin-top|margin-right|margin-bottom|margin-left|padding|padding-top|padding-right|padding-bottom|padding-left|gap|row-gap|column-gap|top|right|bottom|left|inset|inset-block|inset-block-start|inset-block-end|inset-inline|inset-inline-start|inset-inline-end|width|height|min-width|max-width|min-height|max-height|font-size|line-height|border-radius|border-top-left-radius|border-top-right-radius|border-bottom-left-radius|border-bottom-right-radius)[\s\t]*:[\s\t]*([^;{}]*)/gi;
  ...
  while ((propertyMatch = propertyRegex.exec(text)) !== null) {
    ...
    while ((valueMatch = unitRegex.exec(propertyValue)) !== null) {
      ...
      if (originalNumber !== parseFloat(formattedValue)) {
        ...
        const valueStartInDocument =
          propertyMatch.index +
          propertyMatch[0].indexOf(
            valueMatch[0],
            propertyMatch[0].indexOf(":")
          );
        const startPos = document.positionAt(valueStartInDocument);
        const endPos = document.positionAt(
          valueStartInDocument + valueMatch[0].length
        );
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const saveDisposable = vscode.workspace.onWillSaveTextDocument((event) => {
    ...
    if (!validLanguages.includes(document.languageId)) {
      console.log(
        `Type de document non pris en charge: ${document.languageId}`
      );
      return;
    }
